...
class Args:
    ...
    """the discount factor gamma for the intrinsic rewards"""
...
class FeatureNetwork(nn.Module):
    ...
    def update_from_value_net(self, value_net):
        ...
        value_layers = [layer for layer in value_net if isinstance(layer, nn.Linear)][:len(feature_layers)]

        for feat_layer, value_layer in zip(feature_layers, value_layers):
            for feat_param, value_param in zip(feat_layer.parameters(), value_layer.parameters()):
                if feat_param.shape == value_param.shape:
                    ...
                    feat_param.data.copy_(
                        self.tau * value_param.data + (1.0 - self.tau) * feat_param.data
                    )
