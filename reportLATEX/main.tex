...
In \eqref{eq:value_func}, $\gamma$ describes the discount factor and $F\left(s_{t+1}, \textbf{z}\right) =  \boldsymbol{\phi}\left(s\right) \cdot \textbf{z}$ where $\boldsymbol{\phi}\left(s\right): \mathcal{S} \rightarrow \mathbb{R}^{d}$ is a feature extraction network. Both $\boldsymbol{\phi}$ and $\textbf{z}$ are $d$-dimensional vectors.

\texttt{A few sentences placing the work in high-level context. Limit it to a few paragraphs at most; your report is on reproducing a piece of work, you don’t have to motivate that work.}
