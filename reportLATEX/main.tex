...
The obtained $\mathbf{z}$, in conjunction with the processed features, are fed into the reward network that acts as a substitute for the dot product in the paper's \textsc{RLE} network. Therefore, the reward network will operate with the respective features and the $\mathbf{z}$-vector to produce an intrinsic reward to guide exploration. For the construction of the losses we refer to \hyperlink{na-vmf}{Appendix H}.
