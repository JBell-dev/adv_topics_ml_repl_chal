...
\usepackage{array}
\usepackage{float}
\usepackage[a4paper, left=0.25in, right=0.25in, top=1in, bottom=0.25in]{geometry}
...
With respect to \textsc{IsaacLab}, the experiments were run on an Asus TUF Gaming A17 with a RTX 3060 Laptop GPU. The GPU characteristics are below the suggested minimum requirements for VRAM memory of NVIDIA, however, for developing the experiments for the \textsc{CartPole} environment it was sufficient. The used CUDA version was $11.8$ and the project packages were managed through conda besides the separate installation of Omniverse for \textsc{IsaacSim} and the posterior \textsc{IsaacLab} installation (Windows distribution). 
...
The results from our \textsc{IsaacLab} replication show similar results to those of the authors. The \textsc{RLE} algorithm shows the most stable performance in the \textsc{CartPole} environment, see figure \ref{fig:cartpole-return} which replicates figure $28$ and $22$ of the paper. The difference in the maximum score comes from the maximum length of the episodes which are kept at $299$ steps in our replication for computational reasons, \textcolor{red}{besides that is not (did you mean \textit{and besides that, this is not}?)} directly manipulable in the environment wrapper as in the previous \textsc{IsaacGym}. Notice that we also use the same hyperparameters as the authors' implementation for \textsc{PPO}, under the default set of parameters of SKRL so the results can differ from the authors.
...
The observed results from the adaptive VMF are promising (see figure \ref{fig:vmf-game-score}), with the comparable scores visible in table \ref{tab:atari-results-vmf}. In addition, trajectories were the longest for NA-VMF and equally long with respect to \textsc{RLE} for QVMF, see figure \ref{fig:vmf-trajectory-length}. This implies that in the Q-VMF case the agent was able to kill more aliens relative to the other algorithms. Therefore, both of our adaptive VMF implementations outperforms the other algorithms for \textsc{Alien-V5}, pointing us to further investigate this approach for other \textsc{Atari} games and continuous action spaces.  
