...
%Background
\fill[white] (-12.7, -3.2) rectangle (18, 5.7);

% Input nodes
%\node[tensor] (obs) {observation $\in \mathcal{R}^o$};
%\node[tensor, right=of obs] (goal) {z $\in \mathcal{R}^d$};

\node[tensor, anchor=east] (obs) at (-10, -2.5) {observation $\in \mathcal{R}^o$};
\node[tensor, anchor=east] (goal) at (-10, 2.5) {z $\in \mathcal{R}^d$};

% Hidden layers
%\node[op, below=of obs] (network) {Network};
%\node[op, below=of goal] (goal_encoder) {Goal Encoder};

%\node[tensor, below=of network] (obs_hidden) {observation hidden $\in \mathcal{R}^h$};
%\node[tensor, below=of goal_encoder] (goal_hidden) {z hidden $\in \mathcal{R}^h$};

\node[op] (network) at (-7.5, -2.5) {Network};
\node[op] (goal_encoder) at (-7.5, 2.5) {Goal Encoder};

\node[tensor, anchor=west] (obs_hidden) at (-5, -2.5) {observation hidden $\in \mathcal{R}^h$};
\node[tensor, anchor=west] (goal_hidden) at (-5, 2.5) {z hidden $\in \mathcal{R}^h$};

% Element-wise addition 1
%\node[op, below=of obs_hidden, xshift=1.5cm] (element_wise_addition_1) {Element-wise addition};
%\node[tensor, below=of element_wise_addition_1] (hidden) {hidden $\in \mathcal{R}^h$};

\node[op] (element_wise_addition_1) at (-3.16, 0) {Element-wise addition};
\node[tensor, anchor=west] (hidden) at (-0.2, 0) {hidden $\in \mathcal{R}^h$};

% Actor Branch
%\node[op, below left=of hidden] (actor) {Actor};
%\node[tensor, below=of actor] (logits) {logits $\in \mathcal{R}^a$};

\node[op] (actor) at (8.055, -2.5) {Actor};
\node[tensor, anchor=west] (logits) at (15, -2.5) {logits $\in \mathcal{R}^a$};

% Extra Layer Branch
%\node[op, right=2cm of hidden] (extra_layer) {Extra Layer};
%\node[tensor, below=of extra_layer] (features) {features $\in \mathcal{R}^h$};
%\node[op, below=of features] (element_wise_addition_2) {Element-wise addition};
%\node[tensor, below=of element_wise_addition_2] (hidden_2) {hidden 2 $\in \mathcal{R}^h$};

\node[op] (extra_layer) at (0.832, 2.5) {Extra Layer};
\node[tensor, anchor=west] (features) at (3.332, 2.5) {features $\in \mathcal{R}^h$};
\node[op] (element_wise_addition_2) at (8.5, 2.5) {Element-wise addition};
\node[tensor, anchor=west] (hidden_2) at (11.328, 2.5) {hidden 2 $\in \mathcal{R}^h$};

% Critic Branch
%\node[op, below left=of hidden_2] (critic_ext) {Extrinsic Critic};
%\node[tensor, below=of critic_ext] (value_ext) {extrinsic value $\in \mathcal{R}$};
%\node[op, below right=of hidden_2] (critic_int) {Intrinsic Critic};
%\node[tensor, below=of critic_int] (value_int) {intrinsic value $\in \mathcal{R}$};

\node[op] (critic_ext) at (12.5, 0) {Extrinsic Critic};
\node[tensor, anchor=west] (value_ext) at (15, 0) {extrinsic value $\in \mathcal{R}$};
\node[op] (critic_int) at (12.5, 5) {Intrinsic Critic};
\node[tensor, anchor=west] (value_int) at (15, 5) {intrinsic value $\in \mathcal{R}$};
