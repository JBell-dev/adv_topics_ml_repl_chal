# Adjusted Code 2: Ensuring game_score Consistency with Code1
...
import wandb  # Ensure wandb is installed and properly set up
...
if __name__ == "__main__":
    ...
    avg_returns = deque(maxlen=128)    # For normalized IQM
    avg_returns1 = deque(maxlen=128)   # For game_score (raw scores)
    ...
    record_video = False  # Video recording flags
    recording = True
    ...
    for update in range(1, num_updates + 1):
        ...
        agent.sample_noise()  # Sample noise for NoisyNet
        ...
        for step in range(0, args.num_steps):
            ...
            for idx, d in enumerate(done):
                if info["terminated"][idx] or info["TimeLimit.truncated"][idx]:
                    ...
                    avg_returns.append(normalized_score)
                    avg_returns1.append(info["r"][idx])  # Update raw scores deque
                    avg_ep_lens.append(info["l"][idx])
                    # Debugging Statements
                    print(f"Update {update}, Episode {len(avg_returns1)}: Raw Score = {info['r'][idx]}, Normalized Score = {normalized_score}")
        ...
        if args.eval_interval != 0 and update % args.eval_interval == 0:
            ...
            # Rollout the environments until desired episodes are completed
            while len(eval_scores) < args.num_eval_episodes:
                ...
                for idx, d in enumerate(eval_done):
                    if eval_info["terminated"][idx] or eval_info["TimeLimit.truncated"][idx]:
                        # Normalize the score
                        normalized_eval_score = (eval_info["r"][idx] - random_score) / (human_score - random_score)
                        eval_scores.append(normalized_eval_score)
            ...
            print(f"Step {update}: game score (IQM): {np.mean(eval_scores)}")
            ...
            eval_game_score = np.mean(eval_scores)  # Or use median
            ...
            eval_data["eval/time"] = it_end_time - it_start_time
        ...
        # Compute IQM and its confidence interval
        iqm = interquartile_mean(avg_returns_array)
        iqm_lower, iqm_upper = bootstrap_iqm_confidence_interval(avg_returns_array, num_bootstrap=1000, confidence=0.95)

        # Compute Game Score (Mean) and its confidence interval
        game_score = np.mean(avg_returns_array1)
        game_score_lower, game_score_upper = bootstrap_confidence_interval(avg_returns_array1, num_bootstrap=1000, confidence=0.95)

        it_end_time = time.time()

        # Prepare data dictionary with both IQM and Game Score
        data = {}
...
#ADDED BY 
        ...
        # Game Score Logging
        data["charts/game_score"] = game_score
        data["charts/game_score_lower"] = game_score_lower
        data["charts/game_score_upper"] = game_score_upper

        data["charts/max_game_score"] = np.max(avg_returns1, initial=0)
        data["charts/min_game_score"] = np.min(avg_returns1, initial=0)
    ...
    envs.close()  # Ensure environments are closed after all updates

    # Define the path in Google Drive
    save_path = "/content/drive/My Drive/colab_saved_models/saved_rle_networks_new"
