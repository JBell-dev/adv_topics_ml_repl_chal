...
class RLEModel(nn.Module):
    def __init__(self, input_shape, feature_size, output_size, num_actions, num_envs, z_layer_init, device):
        ...
        #another thing, we could test to smooth out the kappa with the global step instead (like global step / trajectory length) 
    ...
    def get_current_direction(self):
        """we try to base in successful experiences to get mu without using NNs"""

        #here we sample the e1 if we do not have any sample good amount experiences to base on
    ...
    def update_sampling_history(self, env_idx, episode_return):
        ...
        # we still in the "learning" phase
        if len(self.success_memory) < self.min_samples_for_exploitation:
        ...
        if len(self.returns_memory) > 0:
            ...
            if episode_return > threshold:
                # so if return higher than that, then good z, keep
